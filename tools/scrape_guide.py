...
def build_step(prefix, position, name, loc, url, item_map, quest_map, cumulative, total_needed):
...
def main():
    ...
    for (order, slug, display, prefix, url, raw) in sections:
        steps = [build_step(prefix, pos, name, loc, u, item_map, quest_map, cumulative, total_needed)
