...
WorldPoint of the quest's FIRST actionable step -- the first step-constructor
(NpcStep / ObjectStep / DetailedQuestStep / ItemStep / ...) in the helper source
that carries a location. The location is either an inline `new WorldPoint(x,y,z)`
OR a local/field WorldPoint variable (e.g. `locationBottomOfGrandTree`) that the
file assigns from a `new WorldPoint(x,y,z)` literal. Step constructors live in
setupSteps()/loadSteps(), AFTER setupZones()/setupRequirements(), so Zone-corner
and requirement WorldPoints are skipped. The first such step is the
...
def load_quest_display():
    with open(QUEST_JAVA, "r", encoding="utf-8") as fh:
        text = fh.read()
    return {m.group(1): m.group(2) for m in QUEST_RE.finditer(text)}
...
STRLIT = re.compile(r'"((?:[^"\\]|\\.)*)"')
QUEST_REF = re.compile(r'\bQuest\.([A-Z][A-Z0-9_]*)')
...
def parse_qhq_entries():
    with open(QHQ, "r", encoding="utf-8") as fh:
        lines = fh.readlines()
    start = next(i for i, ln in enumerate(lines) if "enum QuestHelperQuest" in ln)
    entries, cur = [], None
    i = start + 1
    while i < len(lines):
        ln = lines[i]
        ...
        if m:
            ...
            s = ln.strip()
            if s == ";" or s.startswith("private ") or s.startswith("@"):
                entries.append(cur)
                cur = None
                break
            cur["text"] += ln
...
WPVAR_ASSIGN = re.compile(
    r'\b([A-Za-z_]\w*)\s*=\s*new\s+WorldPoint\s*\('
    r'\s*(-?\d+)\s*,\s*(-?\d+)\s*(?:,\s*(-?\d+)\s*)?\)')
...
def build_wp_vars(text):
    wp = {}
    for m in WPVAR_ASSIGN.finditer(text):
        name = m.group(1)
        x, y = int(m.group(2)), int(m.group(3))
        z = int(m.group(4)) if m.group(4) is not None else 0
        wp.setdefault(name, (x, y, z))  # first assignment wins
    return wp


def first_step_worldpoint(path):
    """Return (x, y, plane, step_class, wp_line, how) or None.
    'how' in {inline, var:<name>}."""
    with open(path, "r", encoding="utf-8", errors="replace") as fh:
        text = fh.read()
    wp_vars = build_wp_vars(text)
    varpat = None
    if wp_vars:
        varpat = re.compile(r'\b(' + "|".join(re.escape(k) for k in wp_vars) + r')\b')
    for m in STEP_CTOR.finditer(text):
        open_idx = m.end() - 1
        inner, _end = extract_balanced(text, open_idx)
        cand = []
        wi = WP_RE.search(inner)
        if wi:
            z = int(wi.group(3)) if wi.group(3) is not None else 0
            cand.append((wi.start(), (int(wi.group(1)), int(wi.group(2)), z),
                         "inline", open_idx + 1 + wi.start()))
        if varpat:
            vm = varpat.search(inner)
            if vm:
                x, y, z = wp_vars[vm.group(1)]
                cand.append((vm.start(), (x, y, z), "var:" + vm.group(1),
                             open_idx + 1 + vm.start()))
        if cand:
            cand.sort(key=lambda c: c[0])  # earliest position within the step
            _pos, (x, y, z), how, abspos = cand[0]
            step_cls = m.group(0)[4:].strip().rstrip("(").strip()
            return x, y, z, step_cls, line_of(text, abspos), how
    return None
...
def main():
    ...
    result, provenance, skipped, non_quest = {}, {}, [], 0
    ...
    for e in entries:
        ...
        if fw is None:
            skipped.append((disp, "no-locatable-step:" + cls))
            continue
        x, y, z, step_cls, wp_line, wphow = fw
        if not sane(x, y, z):
        ...
        if disp in result and result[disp] != [x, y, z]:
            ...
            continue
        result[disp] = [x, y, z]
        rel = os.path.relpath(path, QH_ROOT).replace("\\", "/")
        provenance[disp] = (cls, rel, wp_line, step_cls, wphow)
    ...
    for name, reason in skipped:
        p("     SKIP:", name, "->", reason)
    p("  resolved via WorldPoint variable:",
      sum(1 for v in provenance.values() if v[4].startswith("var:")))
    with open(os.path.join(HERE, "quest_start_coords_provenance.txt"), "w", encoding="utf-8") as fh:
        for disp in sorted(provenance):
            cls, rel, wp_line, step_cls, wphow = provenance[disp]
            fh.write('%-52s %s   [%s %s  %s:%d]\n' %
                     ('"%s"' % disp, result[disp], step_cls, wphow, rel, wp_line))
    p("Wrote", OUT_JSON)
